...
class LLModel:
    ...
    def load_existing_results(self, csv_filename):
        ...
        try:
            ...
            if results:
                ...
                logging.info(f"Found existing results: {completed_count} total completed tests, {unique_cves} unique CVEs in {csv_filename}")
                
                if completed_count > unique_cves:
                    logging.info(f"Note: CSV contains {completed_count - unique_cves} duplicate entries (retries)")
    ...
    def cve_based_challenge_full_dataset(
        ...
        if completed_count > 0:
            logging.info(f"Resuming from position {completed_count}. {len(remaining_dataset)} entries remaining out of {len(cve_dataset)} total")
        else:
        ...
        with logging_redirect_tqdm():
            ...
            with open(csv_filename, mode=file_mode, encoding="utf-8") as csvfile:
                ...
                for entry in tqdm.tqdm(remaining_dataset, desc="Benchmark in progress..."):
                    retries = 0
                    success = None
                    ...
                    while retries <= max_retries and success is None:
                        try:
                            ...
                            if retries <= max_retries:
                                ...
                                logging.error(f"All attempts failed for CVE {entry['cve_id']}: {err}")
                                raise err
...
class ExperimentRunner:
    ...
    def run_single_experiment(self, experiment_config):
        ...
        rate_limiter = RateLimiter(requests_per_minute=rate_limit_config["requests_per_minute"])
        ...
        if max_samples and max_samples < len(dataset):
            ...
            logging.info(f"Limited dataset to {max_samples} samples for experiment {experiment_name}")
        ...
        logging.info(f"Completed experiment: {experiment_name} - Accuracy: {accuracy:.4f}")
        return {
    ...
    def run_all_experiments(self):
        ...
        all_results = []
        enabled_experiments = [exp for exp in self.config["experiments"] if exp["enabled"]]
    ...
    def save_overall_summary(self, all_results):
        ...
        for result in all_results:
            print(f"{result['experiment_name']:<30} {result['model_name']:<30} "
